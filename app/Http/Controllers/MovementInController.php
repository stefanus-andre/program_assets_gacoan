...
class MovementInController extends Controller
{
    ...
    public function HalamanMoveIn(Request $request)
    {
        $reasons = DB::table('m_reason')->select('reason_id', 'reason_name')->get();
        $restos = DB::table('master_resto')->select('store_code', 'resto')->get();
        $approvals = DB::table('mc_approval')->select('approval_id', 'approval_name')->get();
    ...
    public function filter(Request $request)
    {
        ...
        $results = MasterMoveIn::whereBetween('create_date', [$startDate, $endDate]);
    ...
    {
    $moveout = MasterMoveIn::find($id); // Fetch the moveout record by ID
    ...
    public function AddDataMoveOut(Request $request)
    {
        ...
        try {
            // Buat instance dari model MasterMoveIn
            $moveout = new MasterMoveIn();
            ...
            $maxMoveoutId = MasterMoveIn::max('out_no'); // Ambil nilai out_no maksimum
    ...
    public function updateDataMoveIn(Request $request, $id)
    {
        // Validasi input
        $request->validate([
        ...
        return response()->json([
            ...
            'redirect_url' => route('Admin.movein'), // Sesuaikan dengan route index Anda
        ]);
    }

    public function edit($id)
    {
        $moveout = MasterMoveIn::with('asset')->findOrFail($id); // Assuming MoveOut has a relationship with Asset
        return response()->json($moveout);
    }

    public function deleteDataMoveOut($id)
    {
        $moveout = MasterMoveIn::find($id);
        ...
        if ($moveout) {
            ...
            return response()->json([
                ...
                'redirect_url' => route('Admin.movein')
            ]);
        } else {
    ...
    {
        $moveout = MasterMoveIn::where('out_id', $MoveOutId)->first();
