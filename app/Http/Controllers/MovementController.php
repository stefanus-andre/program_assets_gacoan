...
class MovementController extends Controller
{
    ...
    public function updateDataAmo1(Request $request, $id)
    {
        ...
        if ($request->appr_1 == '2') {
            ...
            foreach ($details as $detail) {
                ...
                $newQty = max(0, $detail->qty - 1); // Ensure qty doesn't go below 0
                DB::table('t_out_detail')
                    ->where('id', $detail->id)
                    ->update([
                        'qty' => $newQty,
                        'qty_final' => $detail->qty, // Store original qty in qty_final
                    ]);
        ...
        if ($moveout->save()) {
            return response()->json([
                ...
                // Explicitly define the full URL path for the redirection
                'redirect_url' => url('/admin/apprmoveout-am'), // Ensure correct full URL
            ]);
        } else {
    ...
    public function updateDataAmo3(Request $request, $id)
    {
        try {
            ...
            // Generate a unique in_id for t_out
            $new_in_id = null;
            $new_tf_code = null;
            do {
                ...
                // Check if the generated in_id already exists in t_in
                ...
                // Increment the transaction_number if the in_id exists
                if ($existing_in_id && $existing_tf_code) {
                    $transaction_number++;
                }


            } while ($existing_in_id && $existing_tf_code);  // Repeat the loop until we find a unique in_id
    
            $moveout->in_id = $new_in_id;
            $moveout->tf_code = $new_tf_code;
    
            // If the approval status is '2' and 'appr_2' is also '2', insert into t_in
            if ($request->appr_3 == '2' && $moveout->appr_2 == '2') {
                ...
                // Insert into t_in_detail with proper checks for duplicates
                $moveoutDetails = DB::table('t_out_detail')->where('out_id', $moveout->out_id)->get();
    
                // Track unique in_id per asset tag group
                $previous_asset_tag = null;
                foreach ($moveoutDetails as $index => $detail) {
                    // Generate a new in_id if asset_tag changes (this separates groups of records)
                    if ($previous_asset_tag !== $detail->asset_tag) {
                        // Increment transaction_number for new asset_tag group
                        $transaction_number++;  // New transaction for this group
    
                        // Create a new in_id for this asset_tag group
                        $transaction_number_str = str_pad($transaction_number, 3, '0', STR_PAD_LEFT);
                        $new_in_id = "{$trx_code}.{$today}.{$request->input('reason_id')}.{$request->input('from_loc_id')}.{$transaction_number_str}";
                        $previous_asset_tag = $detail->asset_tag; // Track the asset_tag
                    }
    
                    // Check if the same data already exists in t_in_detail based on in_id and asset_tag
                    $existing_detail = DB::table('t_in_detail')
                        ->where('in_id', $new_in_id)
                        ->where('asset_tag', $detail->asset_tag)
                        ->exists();
    
                    // Insert the detail if it doesn't exist already
                    if (!$existing_detail) {
                ...
                foreach ($moveoutDetails as $detail) {
                    ...
                    ->join('t_out_detail', 't_out_detail.asset_id', '=', 'table_registrasi_asset.id')
                    ->where('table_registrasi_asset.register_code', $detail->asset_tag)
                    ->value('table_registrasi_asset.qty');
                
            
                    // Update qty back to 1
                    if ($currentQty !== 1) {
                        DB::table('table_registrasi_asset')
